...
def collect_trades_mfe_percentile_exit(df, holding_period, mfe_target):
    ...
    for i in range(1, len(df) - holding_period):
        ...
        for j in range(len(highs)):
            ...
            if highs.iloc[j] > max_price:
                max_price = highs.iloc[j]

            # PROFIT TARGET
            if highs.iloc[j] >= target_price:
...
for ticker in tickers:
    ...
    for hp in holding_periods:
        ...
        for label, mfe_target in mfe_targets.items():
            ...
            trades = collect_trades_mfe_percentile_exit(df, hp, mfe_target)
...
agg_results = results_df.groupby(['HP', 'MFE_target']).agg({
    'Win_rate_%': 'mean',
    'Expectancy': 'mean',
    "mean_pos_ret": "mean",
    "mean_neg_ret": "mean",
...
------------
# Test chandelier (largo) + tp su MFE based (alto)
------------
...
holding_periods = [3,4,5,6,7,10,15,25]
...
def collect_trades_chandelier_and_mfe_target(df, holding_period, atr_period, atr_mult, mfe_target):
    ...
    for i in range(1, len(df) - holding_period):
        ...
        # chandelier level
        c_level = (
            abs(chandelier_level - entry_price) / entry_price
            if chandelier_level is not None
            else np.nan
        )
...
def compute_metrics(trades):
    ...
    metrics = {
        ...
        "avg_tp_distance_%": df["take_profit_level"].mean(),
        "avr_chandelier_level": df["c_level"].mean(),
    }
...
-------------------
## Larry exit
#at the first occasion the price at open is in profit
-------------------
